...
import { decrementQuantity, emptyCart, incrementQantity, removecartItem } from '../redux/slices/cartSlice';
const Cart = () => {
              ...
              <div className='grid grid-cols-3 gap-4 mt-5'>
                    ...
                    <tbody>
                      {
                        ...
                        <td>
                          <div className='flex'>
                            <button onClick={()=>handleDecrementQuantity(product)} className='font-bold mr-2'>-</button>
                            <input style={{ width: "40px" }} type="text" className='border p-1 rounded mx-2' value={product?.quantity} readOnly />
                            <button onClick={()=>dispatch(incrementQantity(product?.id))} className='font-bold mr-2'>+</button>
                          </div>
                        </td>
                        <td>{product?.totalPrice}</td>
                        <td><button onClick={()=>dispatch(removecartItem (product?.id))} className='text-red-600 ml-2
                ...
                <div className='col-span-1'>
                  <div className='border rounded shadow p-5'>
                    <h2 className='text-2xl font-bold'>Total Amount: <span className='text-red-600 me-4'>${cartTotal}</span></h2>
                    <hr />
